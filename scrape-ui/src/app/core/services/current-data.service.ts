...
import { CurrentMonthDto } from '../../shared/models/current-month.dto.interface';
...
export class CurrentDataService {
  ...
  currentMonthData$: Observable<CurrentMonthDto[]> = this.fetchCurrentWeekData$.pipe(
    switchMap((_) => this.http.get<CurrentMonthDto[]>(`${this.baseUrl}/api/Averages/daily`)
      .pipe(catchError((_) => of(_)))),
    shareReplay()
  );
}
